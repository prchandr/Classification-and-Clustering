...
# Make array of layer sizes to test. Powers of 2.
layerSizes = np.power(2, range(7))
...
def main():
	...
	# Make classifier lists
	wf_CLF = [MLPClassifier(hidden_layer_sizes=(layerSize), activation='logistic') for layerSize in layerSizes]
	tn_CLF = [MLPClassifier(hidden_layer_sizes=(layerSize), activation='logistic') for layerSize in layerSizes]
	bn_CLF = [MLPClassifier(hidden_layer_sizes=(layerSize), activation='logistic') for layerSize in layerSizes]

	print("\nWaveform: ")
	kFoldTest(wf_data, wf_labels, wf_CLF, kf, printEachVal=True)

	print("\nTwo Norm: ")
	kFoldTest(tn_data, tn_labels, tn_CLF, kf, printEachVal=True)

	print("\nBanana: ")
	optimal_bn_CLF = kFoldTest(bn_data, bn_labels, bn_CLF, kf, printEachVal=True)
	...
	print("\nWaveform PCA-2D: ")
	optimal_wf_CLF = kFoldTest(wf_reduced_data, wf_labels, wf_CLF, kf, printEachVal=True)
